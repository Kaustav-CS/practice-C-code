...
int solveSuduko(int grid[N][N], int row, int col)
{
    ...
    // and 9th column (0 indexed matrix) , we are
    ...
    // of the grid already contains value >0, we iterate for next column
    if (grid[row][col] > 0)
    ...
    for (int num = 1; num <= N; num++)
    {
        ...
        if (isSafe(grid, row, col, num)==1)
        {
            /* assigning the num in the current (row,col)
               position of the grid and assuming our assined num
...
int main()
{
    ...
    return 0;
    // This is code is contributed by Book
}